...
//该函数完成将（部分关心的）配置信息到对应的变量值的实际转换
static int config_insert(server *srv) {
	size_t i;
	int ret = 0;
	buffer *stat_cache_string;
		//cv里列出的都是Lighttpd主程序关心的配置值。还有很多配置信息是主程序并不关心的，比如属于某一特定插件的配置信息。
	config_values_t cv[] = {
		...
		{ "server.host",                 "use server.bind instead", T_CONFIG_DEPRECATED, T_CONFIG_SCOPE_UNSET }, //此项（包括此项）以下是已经被摈弃的配置选项，提示使用更新的选项
		...
		{ NULL,                          NULL, T_CONFIG_UNSET, T_CONFIG_SCOPE_UNSET } //在循环遍历该数组的时候，最后这个元素用作数组结束标记判断
	};


	/* 0 */ //下面这些值的赋值将cv项目和配置变量指向同一个位置，从而对cv项的赋值就是对应配置变量的赋值，从而做到当该函数退出的时候，就算cv被清除掉（cv是局部变量），但从配置值转换而来的配置变量值依旧可以通过相应配置变量来访问到，
	        //达到预定目的。这里是基本的全局配置信息，对应的设置在srv->srvconf结构体各字段内。
...
//connection结构体con根据当前连接状态获取条件配置值
...
//当前连接状态与条件配置信息块的条件配置匹配时才获取对应的配置值，否则直接continue跳过。
...
//对配置文件的语法分析，获得的所有的配置信息存入srv->config_context里
...
//对结构体t的初始化 将filename,s.start,s.size放入结构体t
...
int config_read(server *srv, const char *fn) {
	config_t context; //该结构体在configfile.h中定义
	/*
		typedef struct {
			server *srv;
			int     ok;
			array  *all_configs;
			array  *configs_stack; // to parse(解析) nested(嵌套的) block 
...
//all_configs指向config_context，即使all_configs和config_context指向同一块内存，因此该函数后面对all_configs的修改就是对config_context的修改	
	context.all_configs = srv->config_context;

//strrchr() 函数查找字符在指定字符串中从正面开始的最后一次出现的位置，如果成功，则返回指定字符最后一次出现位置的地址，如果失败，则返回 false。
	pos = strrchr(fn,
#ifdef __WIN32
			'\\'
#else
...
/*
收集一些默认的配置属性。必须看清楚这个赋值，dc是array结构体context.all_configs的第一个数据元素，
而又context.all_configs = srv->config_context，所以srv->config == srv->config_context->data[0]->value
*/	
	srv->config = dc->value; 
...
//调用config_parse_file读取
	ret = config_parse_file(srv, &context, fn); //解析配置文件
...
//若三个插件mod_indexfile,mod_dirlisting,mod_staticfile已经指定加载的
	if (NULL != (modules = (data_array *)array_get_element(srv->config, "server.modules"))) { 
		...
		/* prepend default modules */ //前置默认模块mod_indexfile
		...
		prepends = (data_array *)configparser_merge_data((data_unset *)prepends, (data_unset *)modules); //将modules合并到prepends并返回prepends.
		buffer_copy_string_buffer(prepends->key, modules->key); //复制key.
		array_replace(srv->config, (data_unset *)prepends); //替换。
		modules->free((data_unset *)modules); //释放。
		modules = prepends; //重新指向。
...
//当用户没有开启任何插件的时候，程序按顺序自动添加mod_indexfile,mod_dirlisting,mod_staticfile。同样保证mod_indexfile添加在最前面
	else { 
		data_string *ds;
...
//调用config_insert(srv)函数 根据配置信息设置程序里对应变量值
...
int config_set_defaults(server *srv) {
	...
	if (buffer_is_empty(s->document_root)) { //用户未设置站点主目录，异常返回
	...
	if (buffer_is_empty(srv->srvconf.changeroot)) { //站点根目录未设置，则s->document_root保存的就是主目录的完整路径
		if (-1 == stat(s->document_root->ptr, &st1)) { //判断主目录是否存在
			log_error_write(srv, __FILE__, __LINE__, "sb",
	...
	buffer_to_lower(srv->tmp_buf); //大写转小写

	if (0 == stat(srv->tmp_buf->ptr, &st1)) {
		int is_lower = 0;

		is_lower = buffer_is_equal(srv->tmp_buf, s->document_root); //srv->tmp_buf已经被转换为小写，如果还相等就说明s->document_root也为小写。is_lower=1;

		/* lower-case existed, check upper-case */
		buffer_copy_string_buffer(srv->tmp_buf, s->document_root);

		buffer_to_upper(srv->tmp_buf); //小写转大写
		...
		if (is_lower && buffer_is_equal(srv->tmp_buf, s->document_root)) { //不论转出大写还是小写 srv->tmp_buf->ptr=s->document_root->ptr总是成立，则说明该个目录与大小写无关
			...
			if (st1.st_ino == st2.st_ino) { //大小写不敏感系统
				/* upper and lower have the same inode -> case-insensitve FS */
	...
	if (srv->srvconf.port == 0) { //设置默认端口，HTTP端口为80，HTTPS端口443
	...
	if (srv->srvconf.event_handler->used == 0) {
		 ...
		 */
		 /*
			用户为对采用何种I/O端口多路复用技术做设置，则选择一个默认最好的选项，event_handler里的选项是按好坏评价来排列的，因此知己选择被系统支持的第一项。
		 */
		srv->event_handler = event_handlers[0].et;
		
		if (FDEVENT_HANDLER_UNSET == srv->event_handler) { //使用FDEVENT_HANDLER_UNSET则说明没有用当前系统所支持的多路复用技术，异常返回。
			log_error_write(srv, __FILE__, __LINE__, "s",
		...
		if (FDEVENT_HANDLER_UNSET == srv->event_handler) { //用户选择不被支持，异常返回。
			log_error_write(srv, __FILE__, __LINE__, "sb",
	...
	if (s->is_ssl) {
		if (buffer_is_empty(s->ssl_pemfile)) { //SSL协议需要PEM文件
			/* PEM file is require */
