 ...
 * size fields are maintained */
 ...
 //在用于指定的伸展树t中寻找key等于i的节点，并返回节点
splay_tree * splaytree_splay (splay_tree *t, int i) { //自顶向下，*t为一直指向树的根节点
    splay_tree N, *l, *r, *y;
    int comp, root_size, l_size, r_size;

    if (t == NULL) return t;
    N.left = N.right = NULL;
    l = r = &N; //使*l,*r都指向结构体变量N
    root_size = node_size(t); //#define node_size splaytree_size #define splaytree_size(x) (((x)==NULL) ? 0 : ((x)->size))
    l_size = r_size = 0;

    for (;;) {
        comp = compare(i, t->key); //#define compare(i,j) ((i)-(j))
        if (comp < 0) {
            if (t->left == NULL) break;
            if (compare(i, t->left->key) < 0) {
                y = t->left;   //原根节点的左子节点给y           /* rotate right */
                t->left = y->right; //而原根节点的左子节点的右子节点给原根节点左子节点
                y->right = t;  //原根节点赋给原根节点的右子节点
...
/*
该函数首先在伸展树t中寻找key为i的节点，若找到则返回该节点；
若没找到，则新建一个节点，key为i，内容为data指向的内容并插入树t中，且该节点返回给用户
*/
