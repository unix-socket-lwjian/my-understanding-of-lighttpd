...
//将某插件注册到server结构体srv的plugins中
static int plugins_register(server *srv, plugin *p) {
	plugin **ps;
//若srv的没有插件，则一次申请多个插件内存空间，避免重复申请，减少内存碎片
...
// 下述的两个宏在Lighttpd处理客户端连接请求时至关重要，因为接口函数分为两类（主程序触发、用户请求事件触发），因此宏也有两个。
...
#define PLUGIN_TO_SLOT(x, y) \
	handler_t plugins_call_##y(server *srv, connection *con) {\
		plugin **slot;\
		size_t j;\
                if (!srv->plugin_slots) return HANDLER_GO_ON;\	//不存在任何接口函数链，直接返回HANDLER_GO_ON
                slot = ((plugin ***)(srv->plugin_slots))[x];\	//获取x类接口链
		if (!slot) return HANDLER_GO_ON;\						//不存在x类接口链，直接返回HANDLER_GO_ON	
		for (j = 0; j < srv->plugins.used && slot[j]; j++) { \	//依顺序调用该x类接口链上的接口函数
			plugin *p = slot[j];\
			handler_t r;\
...
//如果p插件提供有y接口的函数
	if (p->y) { \
//srv－>plugin_slots对应y接口的函数链
		plugin **slot = ((plugin ***)(srv->plugin_slots))[x]; \
//srv－>plugin_slots对应y接口的函数链存储空间不存在，则先创建
		if (!slot) { \
			slot = calloc(srv->plugins.used, sizeof(*slot));\
			((plugin ***)(srv->plugin_slots))[x] = slot; \
		} \
//将p插件提供的y接口函数加入到对应的函数链，该for循环从头到尾寻找该函数链中第一个未使用的存储空间，找到后加入并且退出
...
//若插件没有提供初始化其数据存储空间的函数，则此插件没有存储数据
		else {
			p->data = NULL;
		}
	}
