...
static void data_config_print(const data_unset *d, int depth) {
	...
	if (0 == ds->context_ndx) {
		...
		array_print_indent(depth + 1); //输出depth+1个空格
		fprintf(stdout, "# block %d\n", ds->context_ndx);
	}
	depth ++;

	maxlen = array_get_max_key_length(a); //获取数据元素中key的最长长度
	for (i = 0; i < a->used; i ++) {
	...
	if (ds->childs) { //打印子节点项
		...
		for (i = 0; i < ds->childs->used; i ++) {
			...
			if (NULL == ((data_config *)du)->prev) { //只打印链接的第一项，剩下的将在后面打印
				fprintf(stdout, "\n");
				array_print_indent(depth);
				du->print(du, depth);
				fprintf(stdout, "\n");
